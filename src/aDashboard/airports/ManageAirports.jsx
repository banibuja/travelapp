...
function ManageAirports() {
  ...
  const deleteItem = async (id) => {
    try {
      await axios.delete(`http://localhost:5001/api/airports-delete/${id}`, { withCredentials: true });
      setAirports(airports.filter(item => item.id !== id));
      setMessage('Airport deleted successfully!');
      setTimeout(() => setMessage(''), 3000);
    } catch { setMessage('Error deleting airport.'); }
  };
  ...
  const updateItem = async (id) => {
    try {
      const res = await axios.put(`http://localhost:5001/api/airports-update/${id}`, updatedFields, { withCredentials: true });
      setAirports(airports.map(item => item.id === id ? res.data : item));
      setEditingId(null);
      setMessage('Airport updated successfully!');
      setTimeout(() => setMessage(''), 3000);
    } catch { setMessage('Error updating airport.'); }
  };

  const startEditing = (item) => {
    setEditingId(item.id);
    setUpdatedFields({ emri: item.emri, akronimi: item.akronimi, shtetiId: item.shtetiId });
  };

  const filtered = airports.filter(item =>
    item.emri?.toLowerCase().includes(searchTerm.toLowerCase()) ||
    item.akronimi?.toLowerCase().includes(searchTerm.toLowerCase())
  );
          ...
          <div className="px-8 py-6" style={{ background: 'linear-gradient(135deg, #1e3a5f 0%, #2d5a87 100%)' }}>
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
              <div className="flex items-center space-x-4">
                ...
                <div>
                  <h2 className="text-2xl font-bold text-white">Manage Airports</h2>
                  <p className="text-cyan-200 text-sm">{airports.length} airports</p>
                </div>
              </div>
              <div className="flex items-center space-x-3">
                <div className="relative">
                  <input type="text" placeholder="Search airports..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)}
                ...
                <button onClick={() => navigate('/dashboard/AddAirport')}
          ...
          <div className="p-8">
              ...
              <table className="w-full">
                <thead>
                  <tr style={{ background: 'linear-gradient(135deg, #f8fafc, #f1f5f9)' }}>
                    <th className="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase">Airport Name</th>
                    <th className="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase">Code</th>
                    <th className="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase">Country ID</th>
                ...
                <tbody className="divide-y divide-gray-100">
                  {filtered.map((item) => (
                    <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        {editingId === item.id ? (
                          <input type="text" value={updatedFields.akronimi} onChange={(e) => handleEditChange(e, 'akronimi')}
                            className="px-2 py-1 border border-gray-300 rounded-lg text-sm w-20 focus:border-cyan-500 focus:outline-none uppercase" />
                        ) : <span className="px-3 py-1 rounded-full text-xs font-semibold bg-cyan-100 text-cyan-700">{item.akronimi}</span>}
                      </td>
                      <td className="px-6 py-4 text-gray-600">{editingId === item.id ? (
                        <input type="number" value={updatedFields.shtetiId} onChange={(e) => handleEditChange(e, 'shtetiId')}
            ...
            {filtered.length === 0 && (
              <div className="text-center py-12">
                ...
                <p className="text-gray-400 text-lg">No airports found</p>
              </div>
            )}
          </div>
        </div>
