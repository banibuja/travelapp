...
function ManageCardsStamboll() {
  ...
  useEffect(() => {
    const fetchCards = async () => {
      try {
        const response = await axios.get('http://localhost:5000/cards', { withCredentials: true });
  ...
  const deleteCard = async (id) => {
    try {
      await axios.delete(`http://localhost:5000/cards-delete/${id}`, { withCredentials: true });
      ...
      reader.onload = async () => {
        ...
        try {
          const response = await axios.put(
            `http://localhost:5000/cards-update/${editingCard.id}`,
