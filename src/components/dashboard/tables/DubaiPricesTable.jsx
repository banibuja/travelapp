...
function DubaiPricesTable() {
  ...
  useEffect(() => {
    const fetchDubaiPrices = async () => {
      try {
        const response = await axios.get('http://localhost:5000/dubai-price', {
  ...
  const deleteDubaiPrice = async (id) => {
    try {
      await axios.delete(`http://localhost:5000/dubai-prices-delete/${id}`, {
        withCredentials: true,
      });
  ...
  const updateDubaiPrice = async (id) => {
    ...
    try {
      const response = await axios.put(`http://localhost:5000/dubai-prices-update/${id}`, updatedPrice, {
        withCredentials: true,
      });
      setDubaiPrices(
  ...
  const addDubaiPrice = async () => {
    ...
    try {
      const response = await axios.post('http://localhost:5000/add-dubai-price', newPrice, {
        withCredentials: true,
      });
